...
function A_create(){//----新增
    ...
    xhr.open("post", "productsInsert.php");
...
function B_create(){//----新增
    ...
    xhr.onload = function(){
        alert(xhr.responseText);
        location.href="getPromotions.php";
    };
    xhr.open("post", "promotionsInsert.php");
...
function init() {
    ...
    $id("B_upFile").onchange = function (e) {//----新增優惠活動上傳圖片鈕
        ...
        let reader = new FileReader();
        reader.onload = function (e) {
            $id("B_newsImage").src = reader.result;
        };
        reader.readAsDataURL(file); //將圖檔資訊讀出以 base64的編碼方式，以便放到img標籤中
    };
    //-------------------修改 B_優惠活動按鈕--------------------
    ...
    $id("B_E_upFile").onchange = function (e) {//----新增優惠活動上傳圖片鈕
        ...
        let reader = new FileReader();
        reader.onload = function (e) {
            $id("B_E_newsImage").src = reader.result;
        };
        reader.readAsDataURL(file); //將圖檔資訊讀出以 base64的編碼方式，以便放到img標籤中
    };
    */
