...
function init() {
    ...
    $id("A_E_one_upFile").onchange = function (e) {//----新增商品上傳圖片鈕
        ...
        let reader = new FileReader();
        reader.onload = function (e) {
            $id("A_E_one_newsImage").src = reader.result;
        };
        reader.readAsDataURL(file); //將圖檔資訊讀出以 base64的編碼方式，以便放到img標籤中
    };
    $id("A_E_two_upFile").onchange = function (e) {//----新增商品上傳圖片鈕
        let file = e.target.files[0]; //取得所選的file物件的參考
        let reader = new FileReader();
        reader.onload = function (e) {
            $id("A_E_two_newsImage").src = reader.result;
        };
        reader.readAsDataURL(file); //將圖檔資訊讀出以 base64的編碼方式，以便放到img標籤中
    };
    $id("A_E_three_upFile").onchange = function (e) {//----新增商品上傳圖片鈕
        let file = e.target.files[0]; //取得所選的file物件的參考
        let reader = new FileReader();
        reader.onload = function (e) {
            $id("A_E_three_newsImage").src = reader.result;
        };
        reader.readAsDataURL(file); //將圖檔資訊讀出以 base64的編碼方式，以便放到img標籤中
    };
